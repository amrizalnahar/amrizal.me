...
<section class="pt-4 pb-8 md:pb-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-wrap items-center gap-2 mb-4">
            ...
            @endif
            <span class="text-xs text-neutral-500 dark:text-neutral-400">{{ $post->published_at?->format('d M Y') }} · {{ ceil(str_word_count(strip_tags($post->localize('content'))) / 200) }} {{ __('public.common.min_read') }}</span>
        </div>
        ...
        @if ($post->author?->name)
            <p class="mt-4 text-lg text-neutral-600 dark:text-neutral-300 text-balance">{{ __('public.blog.written_by') }} {{ $post->author->name }}</p>
        @endif
    </div>
</section>
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mt-10 pt-8 border-t border-neutral-200 dark:border-neutral-800 card-animate" data-delay="2">
            <div class="flex flex-wrap items-center gap-2">
                <span class="text-sm text-neutral-500 dark:text-neutral-400">{{ __('public.common.tag') }}:</span>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            @if ($previousPost)
                <a href="{{ route('blog.show', $previousPost->slug) }}" class="group flex items-center gap-3 text-left">
                    ...
                    <div>
                        <p class="text-xs text-neutral-500 dark:text-neutral-400">{{ __('public.common.previous_article') }}</p>
            ...
            @if ($nextPost)
                <a href="{{ route('blog.show', $nextPost->slug) }}" class="group flex items-center gap-3 text-right sm:flex-row-reverse self-end sm:self-auto">
                    ...
                    <div>
                        <p class="text-xs text-neutral-500 dark:text-neutral-400">{{ __('public.common.next_article') }}</p>
...
<section class="pb-10 md:pb-14">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-xl font-bold text-neutral-900 dark:text-white mb-6">{{ __('public.common.related') }}</h2>
                ...
                <a href="{{ route('blog.show', $related->slug) }}" class="card-animate group block bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-sm hover:shadow-md dark:hover:shadow-lg dark:hover:shadow-white/5 hover:-translate-y-1 transition-all duration-300 overflow-hidden" data-delay="{{ $loop->index + 1 }}">
                    ...
                    <div class="p-4">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            @endif
                            <span class="text-[10px] text-neutral-500 dark:text-neutral-400">{{ ceil(str_word_count(strip_tags($related->localize('content'))) / 200) }} {{ __('public.common.min_read') }}</span>
                        </div>

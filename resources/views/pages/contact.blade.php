...
<section class="pt-32 pb-12 md:pt-40 md:pb-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="max-w-2xl">
            <p class="text-sm font-medium text-primary-600 mb-3 tracking-wide uppercase">Kontak</p>
            ...
            <p class="mt-4 text-lg text-neutral-600 dark:text-neutral-300 text-balance">
                Saya terbuka untuk diskusi seputar analisis sistem, perancangan arsitektur, atau pengembangan produk digital — baik untuk kolaborasi proyek maupun sekadar bertukar pikiran.
            </p>
        </div>
    </div>
</section>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="card-animate" data-delay="1">
                    <h2 class="text-xl font-semibold text-neutral-900 dark:text-white mb-4">Informasi Kontak</h2>
                    <div class="space-y-4">
                        @if ($settings['email'])
                        ...
                        @if ($settings['whatsapp'])
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $settings['whatsapp']) }}" target="_blank" rel="noopener noreferrer" class="flex items-start gap-4 p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 hover:border-primary-600/30 hover:bg-primary-600/5 transition-all group">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-neutral-900 dark:text-white">WhatsApp</p>
                        ...
                        @if ($settings['github'])
                            <a href="https://github.com/{{ $settings['github'] }}" target="_blank" rel="noopener noreferrer" class="flex items-start gap-4 p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 hover:border-primary-600/30 hover:bg-primary-600/5 transition-all group">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-neutral-900 dark:text-white">GitHub</p>
                        ...
                        @if ($settings['linkedin'])
                            <a href="https://linkedin.com/in/{{ $settings['linkedin'] }}" target="_blank" rel="noopener noreferrer" class="flex items-start gap-4 p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 hover:border-primary-600/30 hover:bg-primary-600/5 transition-all group">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-neutral-900 dark:text-white">LinkedIn</p>
                        ...
                        @if ($settings['location'])
                            <div class="flex items-start gap-4 p-4 rounded-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-neutral-900 dark:text-white">Lokasi</p>
            ...
            <div class="lg:col-span-3">
                <div class="card-animate bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-6 md:p-8" data-delay="2">
                    <h2 class="text-xl font-semibold text-neutral-900 dark:text-white mb-6">Kirim Pesan</h2>
                    ...
                    <form action="{{ route('contact.store') }}" method="POST" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                <label for="name" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Nama</label>
                                <input type="text" id="name" name="name" value="{{ old('name') }}" required class="w-full px-4 py-2.5 rounded-md text-sm bg-neutral-100 dark:bg-neutral-800 border @error('name') border-red-500 dark:border-red-500 @else border-neutral-200 dark:border-neutral-700 @enderror text-neutral-900 dark:text-white placeholder:text-neutral-500 focus:border-primary-600 focus:ring-2 focus:ring-primary-600/20 outline-none transition-all" placeholder="Nama lengkap">
                                @error('name')
                            ...
                            <div>
                                ...
                                <input type="email" id="email" name="email" value="{{ old('email') }}" required class="w-full px-4 py-2.5 rounded-md text-sm bg-neutral-100 dark:bg-neutral-800 border @error('email') border-red-500 dark:border-red-500 @else border-neutral-200 dark:border-neutral-700 @enderror text-neutral-900 dark:text-white placeholder:text-neutral-500 focus:border-primary-600 focus:ring-2 focus:ring-primary-600/20 outline-none transition-all" placeholder="nama@email.com">
                                @error('email')
                                    <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div>
                            <label for="subject" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Subjek</label>
                            <input type="text" id="subject" name="subject" value="{{ old('subject') }}" required class="w-full px-4 py-2.5 rounded-md text-sm bg-neutral-100 dark:bg-neutral-800 border @error('subject') border-red-500 dark:border-red-500 @else border-neutral-200 dark:border-neutral-700 @enderror text-neutral-900 dark:text-white placeholder:text-neutral-500 focus:border-primary-600 focus:ring-2 focus:ring-primary-600/20 outline-none transition-all" placeholder="Subjek pesan">
                            @error('subject')
                                <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        <div>
                            <label for="message" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Pesan</label>
                            <textarea id="message" name="message" rows="5" required class="w-full px-4 py-2.5 rounded-md text-sm bg-neutral-100 dark:bg-neutral-800 border @error('message') border-red-500 dark:border-red-500 @else border-neutral-200 dark:border-neutral-700 @enderror text-neutral-900 dark:text-white placeholder:text-neutral-500 focus:border-primary-600 focus:ring-2 focus:ring-primary-600/20 outline-none transition-all resize-none" placeholder="Tulis pesan Anda di sini...">{{ old('message') }}</textarea>
                            @error('message')
                                <p class="mt-1 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        <div class="flex items-center justify-between gap-4">
                            <p class="text-xs text-neutral-500 dark:text-neutral-400">Balasan biasanya dikirim dalam 1-2 hari kerja.</p>

<div>
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('admin.blog') }}" class="text-neutral-500 hover:text-neutral-700">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
            </a>
            <h1 class="text-xl font-bold text-neutral-800">{{ $post ? 'Edit Artikel' : 'Tulis Artikel' }}</h1>
        </div>
        ...
        <form wire:submit="save" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <x-input-label for="title_id" value="Judul" />
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary-600 text-white">ID</span>
                        </div>
                        <x-text-input id="title_id" wire:model="title_id" type="text" class="mt-1 block w-full" placeholder="Masukkan judul artikel" />
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <x-input-label for="title_en" value="Judul" />
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-neutral-100 text-neutral-600">EN</span>
                        </div>
                        <x-text-input id="title_en" wire:model="title_en" type="text" class="mt-1 block w-full" placeholder="Enter article title" />
                    ...
                    <div class="md:col-span-2">
                        ...
                        <div class="flex gap-2 mt-1">
                            ...
                            <button type="button" wire:click="generateSlug" class="px-3 py-2 bg-neutral-100 text-neutral-600 text-sm rounded-md hover:bg-neutral-200 transition-colors shrink-0">Generate</button>
                        </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select id="category_id" wire:model="category_id" class="mt-1 block w-full border-neutral-300 focus:border-primary-600 focus:ring-primary-600 rounded-md shadow-sm text-sm">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                ...
                <div class="flex gap-2 mb-3">
                    ...
                    <button type="button" wire:click="addNewTag" class="px-3 py-2 bg-primary-600 text-white text-sm rounded-md hover:bg-primary-900 transition-colors shrink-0">Tambah</button>
                </div>
                <div class="flex flex-wrap gap-2" x-data="{ selected: @entangle('selectedTags') }">
                    @foreach($tags as $tag)
                        <button type="button"
                            ...
                            :class="selected.map(String).includes('{{ $tag->id }}') ? 'bg-primary-600 border-primary-600 text-white' : 'bg-white border-neutral-200 text-neutral-600 hover:border-neutral-300'"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                ...
                <div class="flex items-start gap-4">
                    ...
                    @elseif($existingThumbnail)
                        <div class="relative">
                            <img src="{{ Storage::url($existingThumbnail) }}" alt="Thumbnail" class="w-32 h-24 rounded-lg object-cover border border-neutral-200">
            ...
            <!-- Content ID with Trix -->
            <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                <div class="flex items-center gap-2 mb-2">
                    <x-input-label for="content_id" value="Konten" />
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary-600 text-white">ID</span>
                </div>
                <div class="mt-1" wire:ignore>
                    <trix-editor
                        x-data
                        x-on:trix-change="$wire.set('content_id', $event.target.value)"
                        x-ref="trix-id"
                        input="trix-content-id"
                        class="trix-content border-neutral-300 focus:border-primary-600 focus:ring-primary-600 rounded-md shadow-sm min-h-[300px]"
                    >{!! $content_id !!}</trix-editor>
                </div>
                <input type="hidden" id="trix-content-id" value="{{ $content_id }}">
                <x-input-error :messages="$errors->get('content_id')" class="mt-1" />
            </div>

            <!-- Content EN with Trix -->
            <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                <div class="flex items-center gap-2 mb-2">
                    <x-input-label for="content_en" value="Konten" />
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-neutral-100 text-neutral-600">EN</span>
                </div>
                <div class="mt-1" wire:ignore>
                    <trix-editor
                        x-data
                        x-on:trix-change="$wire.set('content_en', $event.target.value)"
                        x-ref="trix-en"
                        input="trix-content-en"
                        class="trix-content border-neutral-300 focus:border-primary-600 focus:ring-primary-600 rounded-md shadow-sm min-h-[300px]"
                    >{!! $content_en !!}</trix-editor>
                </div>
                <input type="hidden" id="trix-content-en" value="{{ $content_en }}">
                <x-input-error :messages="$errors->get('content_en')" class="mt-1" />
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-neutral-200 overflow-hidden" x-data="{ open: false }">
                ...
                <div x-show="open" x-collapse class="px-6 pb-6 space-y-4 border-t border-neutral-100 pt-4">
                    ...
                    <div>
                        ...
                        <textarea id="meta_description" wire:model="meta_description" rows="3" class="mt-1 block w-full border-neutral-300 focus:border-primary-600 focus:ring-primary-600 rounded-md shadow-sm text-sm" placeholder="Deskripsi untuk mesin pencari (default: cuplikan konten)"></textarea>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
                ...
                <div class="flex gap-4">
                    ...
                    <label class="inline-flex items-center">
                        <input type="radio" wire:model="status" value="published" class="border-neutral-300 text-primary-600 focus:ring-primary-600">
                        <span class="ml-2 text-sm text-neutral-700">Published</span>
                    </label>
                </div>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('admin.blog') }}" class="px-4 py-2 bg-neutral-100 text-neutral-700 text-sm font-medium rounded-lg hover:bg-neutral-200 transition-colors">Batal</a>
                <x-primary-button type="submit" wire:loading.attr="disabled" wire:target="save,thumbnail">{{ $post ? 'Simpan Perubahan' : 'Tambah Artikel' }}</x-primary-button>
            </div>
        </form>
    </div>
</div>

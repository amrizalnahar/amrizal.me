<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl font-bold text-neutral-800">Pesan Kontak</h1>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="bg-neutral-50 text-neutral-600 font-medium border-b border-neutral-200">
                    <tr>
                        <th class="px-4 py-3 cursor-pointer hover:text-neutral-800" wire:click="sortBy('name')">
                        ...
                        <th class="px-4 py-3 cursor-pointer hover:text-neutral-800" wire:click="sortBy('created_at')">
                ...
                <tbody class="divide-y divide-neutral-100">
                        ...
                        <tr class="hover:bg-neutral-50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.contacts.show', $contact) }}" class="p-1.5 text-neutral-500 hover:text-primary-600 hover:bg-primary-50 rounded-md transition-colors" title="Lihat Detail">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>
                                    @if($contact->status === 'unread')
                                        <button wire:click="markAsRead({{ $contact->id }})" class="p-1.5 text-neutral-500 hover:text-green-600 hover:bg-green-50 rounded-md transition-colors" title="Tandai sudah dibaca">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 19v-8.93a2 2 0 01.89-1.664l7-4.666a2 2 0 012.22 0l7 4.666A2 2 0 0121 10.07V19M3 19a2 2 0 002 2h14a2 2 0 002-2M3 19l6.75-4.5M21 19l-6.75-4.5M3 10l6.75 4.5M21 10l-6.75 4.5m0 0l-1.14.76a2 2 0 01-2.22 0l-1.14-.76"></path></svg>
                                        </button>
                                    @else
                                        <button wire:click="markAsUnread({{ $contact->id }})" class="p-1.5 text-neutral-500 hover:text-primary-600 hover:bg-primary-50 rounded-md transition-colors" title="Tandai belum dibaca">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                        </button>
                                    @endif
                                    <button wire:click="confirmDelete({{ $contact->id }})" class="p-1.5 text-neutral-500 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-neutral-500">Tidak ada pesan kontak.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($deleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center" x-data x-init="">
            ...
            <div class="relative bg-white rounded-xl shadow-lg p-6 max-w-sm w-full mx-4">
                ...
                <p class="text-sm text-neutral-600 mb-4">Yakin ingin menghapus pesan dari "{{ $deleteTitle }}"? Tindakan ini tidak dapat dibatalkan.</p>

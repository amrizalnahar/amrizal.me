        ...
        <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
            <form wire:submit="save">
                ...
                <div class="mt-6 flex items-center justify-between">
                    <div>
                        @if(! $isCreate)
                            <button type="button" wire:click="sendInvitation" wire:loading.attr="disabled" wire:target="sendInvitation" @disabled($invitationSent) class="inline-flex items-center justify-center gap-2 px-4 h-10 text-white text-sm font-medium rounded-md transition-colors disabled:opacity-70 disabled:cursor-not-allowed whitespace-nowrap {{ $invitationSent ? 'bg-emerald-500' : 'bg-emerald-600 hover:bg-emerald-700' }}">
                                <svg wire:loading.remove wire:target="sendInvitation" class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
                                <svg wire:loading wire:target="sendInvitation" class="w-4 h-4 shrink-0 animate-spin" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                <span wire:loading.remove wire:target="sendInvitation">{{ $invitationSent ? 'Undangan Terkirim' : 'Kirim Undangan' }}</span>
                                <span wire:loading wire:target="sendInvitation">Mengirim...</span>
                            </button>
                        @endif
                    </div>

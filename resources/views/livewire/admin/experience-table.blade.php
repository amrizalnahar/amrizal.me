<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl font-bold text-neutral-800">Pengalaman Kerja</h1>
        <a href="{{ route('admin.experiences.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 text-white text-sm font-medium rounded-lg hover:bg-primary-900 transition-colors shadow-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-neutral-200 p-4 mb-4 flex flex-col lg:flex-row gap-3">
        <div class="flex-1">
            <input wire:model.live="search" type="text" placeholder="Cari perusahaan atau posisi..."
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-neutral-50 text-neutral-600 font-medium border-b border-neutral-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 cursor-pointer hover:text-neutral-800" wire:click="sortBy('company_name')">
                            Perusahaan & Posisi {!! $sortField === 'company_name' ? ($sortDirection === 'asc' ? '↑' : '↓') : '' !!}
                        </th>
                        <th class="px-4 py-3 cursor-pointer hover:text-neutral-800" wire:click="sortBy('started_at')">
                ...
                <tbody class="divide-y divide-neutral-100">
                    @forelse ($this->experiences as $experience)
                        <tr class="hover:bg-neutral-50 transition-colors">
                            <td class="px-4 py-3">
                                @if($experience->logo)
                                    <img src="{{ Storage::url($experience->logo) }}" alt="{{ $experience->company_name }}" class="w-10 h-10 rounded-lg object-cover">
                                @else
                            ...
                            <td class="px-4 py-3">
                                <div class="font-medium text-neutral-800">{{ $experience->company_name }}</div>
                                <div class="text-xs text-neutral-500">{{ $experience->position }}</div>
                            </td>
                            <td class="px-4 py-3 text-neutral-600">
                                {{ $experience->started_at->format('M Y') }} — {{ $experience->is_current ? 'Sekarang' : ($experience->ended_at ? $experience->ended_at->format('M Y') : '-') }}
                            </td>
                            <td class="px-4 py-3 text-neutral-600">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.experiences.edit', $experience) }}" class="p-1.5 text-neutral-500 hover:text-primary-600 hover:bg-primary-50 rounded-md transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <button wire:click="confirmDelete({{ $experience->id }})" class="p-1.5 text-neutral-500 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-8 text-center text-neutral-500">Tidak ada data pengalaman kerja.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($deleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center" x-data x-init="">
            ...
            <div class="relative bg-white rounded-xl shadow-lg p-6 max-w-sm w-full mx-4">
                ...
                <p class="text-sm text-neutral-600 mb-4">Yakin ingin menghapus pengalaman kerja "{{ $deleteTitle }}"? Tindakan ini tidak dapat dibatalkan.</p>

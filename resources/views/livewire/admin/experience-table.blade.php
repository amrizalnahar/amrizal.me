<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl font-bold text-gray-800">Pengalaman Kerja</h1>
        <a href="{{ route('admin.experiences.create') }}" class="inline-flex items-center px-4 py-2 bg-[#1A6FAA] text-white text-sm font-medium rounded-lg hover:bg-[#155a8a] transition-colors shadow-sm">
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-50 text-gray-600 font-medium border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 cursor-pointer hover:text-gray-800" wire:click="sortBy('company_name')">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($this->experiences as $experience)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3">
                                <div class="font-medium text-gray-800">{{ $experience->company_name }}</div>
                                <div class="text-xs text-gray-500">{{ $experience->position }}</div>
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.experiences.edit', $experience) }}" class="p-1.5 text-gray-500 hover:text-[#1A6FAA] hover:bg-blue-50 rounded-md transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    <button wire:click="confirmDelete({{ $experience->id }})" class="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-8 text-center text-gray-500">Tidak ada data pengalaman kerja.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($deleteId)
        <div class="fixed inset-0 z-50 flex items-center justify-center" x-data x-init="">
            ...
            <div class="relative bg-white rounded-xl shadow-lg p-6 max-w-sm w-full mx-4">
                ...
                <p class="text-sm text-gray-600 mb-4">Yakin ingin menghapus pengalaman kerja "{{ $deleteTitle }}"? Tindakan ini tidak dapat dibatalkan.</p>

<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl font-bold text-neutral-800">Manajemen Users</h1>
        @can('users-create')
        <a href="{{ route('admin.users.create') }}" class="inline-flex items-center px-4 py-2 bg-primary-600 text-white text-sm font-medium rounded-lg hover:bg-primary-900 transition-colors shadow-sm">
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-neutral-50 text-neutral-600 font-medium border-b border-neutral-200">
                    <tr>
                        ...
                        <th class="px-5 py-3 cursor-pointer hover:text-neutral-800" wire:click="sortBy('email')">
                ...
                <tbody class="divide-y divide-neutral-100">
                    @forelse ($users as $user)
                        <tr class="hover:bg-neutral-50 transition-colors">
                            <td class="px-5 py-3.5 font-medium text-neutral-800">{{ $user->name }}</td>
                            <td class="px-5 py-3.5 text-neutral-500">{{ $user->email }}</td>
                            ...
                            <td class="px-5 py-3.5 text-center">
                                ...
                                <button wire:click="toggleActive({{ $user->id }})"
                                        class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-600 focus:ring-offset-2 {{ $user->is_active ? 'bg-primary-600' : 'bg-neutral-300' }}">
                                ...
                                @else
                                <span class="inline-flex h-5 w-9 items-center rounded-full {{ $user->is_active ? 'bg-primary-600' : 'bg-neutral-300' }}">
                                    <span class="inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform {{ $user->is_active ? 'translate-x-5' : 'translate-x-1' }}"></span>
                                </span>
                                @endcan
                                <span class="block text-xs mt-1 {{ $user->is_active ? 'text-green-600' : 'text-neutral-400' }}">
                            ...
                            <td class="px-5 py-3.5 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @can('users-edit')
                                    <a href="{{ route('admin.users.edit', $user) }}" class="p-1.5 text-neutral-500 hover:text-primary-600 hover:bg-primary-50 rounded-md transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </a>
                                    @endcan
                                    @can('users-delete')
                                    <button wire:click="confirmDelete({{ $user->id }})" class="p-1.5 text-neutral-500 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-8 text-center text-neutral-500">Tidak ada data user.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if($confirmingDelete)
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 z-10 p-6">
                ...
                <p class="text-sm text-neutral-500 mb-6">Apakah Anda yakin ingin menghapus user ini?</p>
